function criarBarra(var_nome, var_avatar) {
  ...
  userBar.className = "barra-de-usuário";

  var backButton = document.createElement("div");
  backButton.className = "voltar";
  ...
  nome.className = "nome";
  name.innerHTML = '<span>'+var_nome+'</span> <span style="float: right" data-testid="psa-verified" data-icon="psa-verified" class=""><svg viewBox="0 0 18 18" height="18" width="18" preserveAspectRatio="xMidYMid meet" class="" version="1.1" x="0px" y="0px" enable-background="new 0 0 18 18" xml:space="preserve"><polygon id="Star-2" fill="#00DA60" points="9,16 7.1,16.9 5.8,15.2 3.7,15.1 3.4,13 1.5,12 2.2,9.9 1.1,8.2 2.6,6.7 2,4,4,6 4,5,4 5,3,2 7,4,2,4 9,1,1 10,7,2,4 12,7,2 13,6,4 15,6,4,6 15,5,6,7 17,8,2 15,9,9,9 16,5,12 14,7,13 14,3,15,1 12,2,15,2 10,9,16,9 "></polygon><polygon id="Ícone de verificação" fill="#FFFFFF" points="13,1,7,3 12,2,6,5 8,1,10,6 5,9,8,5 5,9,4 8,12,4 "></polygon></svg></span>';
  
  nome.appendChild(varStatus)

  var moreActions = document.createElement("div");
  moreActions.className = "ações mais";
  // moreActions.innerHTML = '<i class="zmdi zmdi-more-vert"></i>';

  var attachmentAction = document.createElement("div");
  attachmentAction.className = "ações cam-action";
  ...
  var phoneAction = document.createElement("div"); // Alterado para um elemento do link <a>
  phoneAction.className = "ações phone-action";
  ...
  userBar.appendChild(botãovoltar);
  userBar.appendChild(avatar);
  userBar.appendChild(nome);
  // userBar.appendChild(maisAções);
  userBar.appendChild(ação_telefone);
  userBar.appendChild(açãoAnexo);

  se (elementoPai) {
    elementoPai.prepend(userBar);
  }
const botBody = elementoPai
const userAvatar = botBody.querySelector('img[elementtiming="Avatar do bot"]');
...
deixe mensagensComprimento = 0;

definirIntervalo(() => {
  const isTyping = botBody.querySelector('.bubble1');
  const irmão = isTyping?.parentElement?.parentElement?.nextSibling;

  if(isTyping && irmão.src) {
    status.innerText = 'gravando áudio...'
  } senão if(isTyping) {      
    status.innerText = 'digitando...'
  } outro {
    status.innerText = 'On-line'
  }

const allMessages = botBody.querySelector('.typebot-chat-view').querySelectorAll('.items-start.typebot-host-bubble');    
  
se(allMessages.length > mensagesLength) {        
  se(!estádigitando) {
    para (deixe i = mensagesLength; i < allMessages.length; i++) {
      const iconContainer = document.createElement('div');
      const checkIcon = `<svg id="checkIcon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 38.626 24.684" style="posição: absoluto; inferior: 0; direita: 5px; z-index: 99" altura="20" largura="18">
      ...
      const currentMsg = todas as mensagens [i];
      currentMsg.append(iconContainer);
    }
    
    mensagesLength = todas as mensagens.length;
    audioNot.play();        
  }
}
...
var cssId = 'myCss'; // você pode codificar o próprio caminho css para gerar id.
se (!document.getElementById(cssId))
{
